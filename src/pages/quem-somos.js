...
export function initQuemSomos(data) {
    ...
    if (identitySection) {
        identitySection.innerHTML = html`
            <div class="max-w-container-max mx-auto px-margin-mobile md:px-gutter">
                <div class="flex flex-col md:flex-row gap-gutter items-center">
                    ...
                    <div class="md:w-1/2 flex flex-wrap justify-center gap-4">
                        ${data.reformedIdentity.solas.map(sola => html`
    ...
    if (standardsSection) {
        standardsSection.innerHTML = html`
            <div class="max-w-container-max mx-auto px-margin-mobile md:px-gutter">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-gutter">
                    ${data.faithStandards.map(item => html`
    ...
    if (grid) {
        grid.innerHTML = html`${data.leaders.map(leader => html`
            <div class="bg-surface-container-lowest p-gutter rounded-xl shadow-sm border border-outline-variant/10 text-center hover:shadow-md transition-shadow group">
                <div class="w-32 h-32 mx-auto rounded-full overflow-hidden mb-stack-md ring-4 ring-primary/5 group-hover:ring-secondary/20 transition-all">
                    ${leader.image ? html`<img src="${leader.image}" class="w-full h-full object-cover">` : html`<div class="w-full h-full bg-surface-container-high flex items-center justify-center"><span class="material-symbols-outlined text-outline text-4xl">person</span></div>`}
                </div>
